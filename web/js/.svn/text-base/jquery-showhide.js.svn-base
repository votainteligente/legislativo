...
//filtro por 3 campos (diputado/senador, comuna y nombre)
function superfilter(query, column) {
  query =   $.trim(query); //trim white space  
  query = query.replace(/ /gi, '|'); //add OR for regex query  
  
  $('table.parlamentarios tbody tr').each(function() {  
    if (($(this).find('img.sd').attr('alt').search(new RegExp($('input:radio:checked').val(), "i")) < 0) || 
        ($(this).find('td.comuna').text().search(new RegExp($('#comuna').val(), "i")) < 0) ||
        ($(this).find('img.sp').attr('alt').search(new RegExp($('#partido').val(), "i")) < 0) ||
        ($(this).find('#filter').text().search(new RegExp(query, "i")) < 0)) {
